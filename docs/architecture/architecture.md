# Architecture — AI & Social Features
...
### 2.9 Job Cost Tracking

The `cyoa_ai_jobs` table has cost columns (added in v3 migration):
...
### 2.10 File Structure for AI Features
...
data/                    — Data-driven content (loaded via data.php):
  ...
  play_fonts.json        — Curated, mood-tagged Google-Fonts allow-list (Phase 41)
...
A story's **theme is purely visual**: a body font + heading font (from the Phase 41 allow-list) and
...
- **Presets** live in `data/themes.json`; the font allow-list in `data/play_fonts.json` (read via
  ...
  table is leftover Phase 42 scaffolding.)
...
## 7. Story Image Gallery (v7) — `gallery.php`
