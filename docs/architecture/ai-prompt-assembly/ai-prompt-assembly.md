...
sequenceDiagram
    ...
    W->>Q: save scenes/choices, then queue cover + per-scene image jobs
...
## 11. Using this with NotebookLM

NotebookLM is grounded in the sources you give it, so feed it **this file** (and optionally
`ai-prompts.md` and `architecture.md` from this folder for extra depth). Then ask for:

- **Audio Overview** — a spoken "deep dive" walkthrough of the pipeline (great for a class demo).
- **Mind Map** — a hierarchical breakdown of the stages and data sources.
- **Study guide / FAQ** — e.g. "Explain how guardrails are injected" or "Why is it many calls, not one?"
- A **narrated script** you can pair with the Mermaid diagrams above.

What NotebookLM will **not** do well: produce the boxes-and-arrows data-flow diagram itself — use the
Mermaid blocks in §9–§10 for that (render + export to image), and let NotebookLM handle the
explanation/audio/mind-map layer.

> Tip: the Mermaid diagrams render as real flow/sequence diagrams. If NotebookLM only reads the text,
> consider exporting the diagrams to PNG first (via mermaid.live) and adding those images as sources.
```
